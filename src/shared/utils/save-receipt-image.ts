/**
 * Render a `ReceiptRecord` to a PNG and save it to the device's camera
 * roll (or trigger a browser download on desktop).
 *
 * Strategy:
 *   1. Draw the receipt onto an off-screen high-DPI canvas.
 *   2. Export as PNG blob.
 *   3. `navigator.share({ files })` — on iOS/Android the share sheet
 *      surfaces "Save Image" prominently, which writes to Photos / camera
 *      roll. On browsers that don't support file sharing, fall back to a
 *      plain `<a download>` trigger.
 *
 * The canvas render waits for `document.fonts.ready` so the Google-loaded
 * DM Sans / DM Serif Display / JetBrains Mono families are available;
 * each has a serif/monospace fallback if a load times out.
 */
...
async function renderToCanvas(
  ...
  // ── Height calculation ──────────────────────────────────────────
  // Keep in sync with the draw loop below — each section advances `y`
  // by the same amount as the pre-measurement.
  let h = PAD;
