/**
 * Top-level error boundary. Without it, any runtime error in render
 * unmounts the entire React tree, leaving an empty `#root` — which
 * renders as a black page (the editorial `--color-bg`).
 *
 * Logs to the console and surfaces the failing message + a stack-frame
 * peek as an editorial-styled wall so the user can see what's wrong
 * instead of staring at black.
 */
...
export class ErrorBoundary extends Component<{ children: ReactNode }, ErrorBoundaryState> {
  ...
  componentDidCatch(error: Error, info: ErrorInfo): void {
    ...
    // Forward to Sentry alongside the in-app fallback UI. The
    // `boundary` tag is categorical (closed set: `"root"` for now),
    // and the component stack ships as an extra so the dashboard
    // shows it without it counting as PII.
