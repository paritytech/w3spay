...
/**
 * Dev-only manual-payment surface (four stage views), gated behind
 * `envConfig.features.devPaymentOverride`. NEVER reuse these for the live
 * TSE-scan flow — synthesizing `ParsedTseQr`/`MerchantEntry` here would
...
/**
 * Editorial copy-to-clipboard row. Inline confirmation flips for ~1.5s so the
 * user gets an ack even on webviews that suppress toast UX. Falls back from
 * the Clipboard API to a hidden `<textarea>` + `execCommand("copy")` for
 * older Android system-Chrome webviews.
 */
