...
/**
 * Stage-decision functions for the customer flow: the `AppStage` union plus
 * the pure, React-free functions that map host state / scan outcomes /
 * payment errors onto the next stage. No state, no effects — routing
...
/**
 * Decide which stage to flip to when the grace timer fires. Pure: no React,
...
/**
 * Decide whether a thrown payment is an auth/host failure (→ `needsLogin` /
 * `hostUnavailable`) or a genuine payment failure (→ `payError`). Pure, so
 * the catch branch stays deterministic and testable.
 */
...
/**
 * Map a freshly-loaded merchant table + queued scan into the next routing
 * stage. `null` ⇒ stay in `merchantsLoading` (the table hasn't appeared
 * yet). `tipScreenEnabled` routes to `tip` vs `confirm`.
 */
