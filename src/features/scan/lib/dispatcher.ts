...
export function dispatchScannedPayload(raw: string): ScanResult {
  ...
  if (raw.startsWith("{")) {
    // Leading "{" is JSON. Parse once and sniff `type`: a t3rminal
    // receipt is parsed + saved by the consumer; everything else keeps
    // the legacy "embedded SDK claim" classification so the existing
    // handoff-JSON behaviour is preserved.
    let parsed: unknown;
    try {
